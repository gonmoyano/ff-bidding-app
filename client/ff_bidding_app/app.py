...
# ShotGrid logo - stylized S shape (based on Shotgun Software logo)
SHOTGRID_LOGO_SVG = """<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="{size}" height="{size}">
    <circle cx="12" cy="12" r="10" fill="#FF6B35"/>
    <path d="M8 8 Q12 8 12 12 Q12 16 16 16" stroke="white" stroke-width="3" fill="none" stroke-linecap="round"/>
...
def create_icon_from_svg(svg_content, size=24):
    """Create a QIcon from complete SVG content.
    ...
    formatted_svg = svg_content.format(size=size) if "{size}" in svg_content else svg_content
...
class PackageManagerApp(QtWidgets.QMainWindow):
    ...
    def _create_top_bar(self):
        ...
        load_sg_btn.setIcon(create_icon_from_svg(SHOTGRID_LOGO_SVG, size=20))
